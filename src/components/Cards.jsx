...
function Cards({ data, containerRef, onDelete }) {
  ...
  return (
    <motion.div
      ...
      className={`relative w-[40vh] h-[40vh] rounded-[20px] bg-zinc-900 text-white p-5 overflow-hidden`}
      transition={{ type: "spring", duration: 0.5 }} // Specify the transition type and duration
    >
      <MdAttachFile />
      <p className='text-xs mt-5 font-semibold text-[20px] leading-tight'>{data.desc}</p>
